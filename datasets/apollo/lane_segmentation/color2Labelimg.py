...
def createLabelImage(colorimg, outline=None):
    ...
    for class_dic in range(len(labels)):  # len(trainId2label) = 37
        ...
        class_map = np.all(equality,
                           axis=-1)  # out put a shape[0]*shape[1] array ,and any pix be showe as Ture or False
        # print(class_map)
        semantic_map.append(
            class_map)  # out put a shape[0]*shape[1] array with the number is shape[2] ,and any pix be showa as Ture or False form 0 until the shape[2] be a class number

    labelImg = np.stack(semantic_map,
                        axis=-1)  # out put a shape[0]*shape[1]*shape[2] array and any pix be showa as Ture or False.
