...
tf.app.flags.DEFINE_string('apollo_root',
                           '/home/zgx010/TensorflowModels/models/research/deeplab/datasets/apollo/',
                           'apollo dataset root folder.') #apollo root :./
...
# A map from data type to filename postfix
_POSTFIX_MAP_val = {
    'image': '_Camera_5',  # old:_leftImg8bit filename:aachen_000000_000019_leftImg8bit.png  new: "_Camera_" filename:170927_063811892_Camera_5.jpg
    'label': '5_bin_labelTrainIds', # newfilename:170927_063811892_Camera_5_bin_labelTrainIds.png
}
...
_IMAGE_FILENAME_VALRE = re.compile('(.+)' + _POSTFIX_MAP_val['image'])
...
def _get_files(data, dataset_split):
  ...
  if dataset_split == 'val':
      pattern = '*%s.%s' % (_POSTFIX_MAP_val[data], _DATA_FORMAT_MAP[data])  # _Camera_6.jpg
  else:
      pattern = '*%s.%s' % (_POSTFIX_MAP[data], _DATA_FORMAT_MAP[data]) # _Camera_5.jpg
  search_files = os.path.join(
      FLAGS.apollo_root, _FOLDERS_MAP[data], dataset_split, '*', '*', '*', '*', pattern)  # ./colorimg/train/*/*/*/*/*Cameare_5.jpg.
...
def _convert_dataset(dataset_split):
  ...
  image_reader = build_data.ImageReader('jpg', channels=3)  #new file is jpg
  ...
  for shard_id in range(_NUM_SHARDS):
    ...
    with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
      ...
      for i in range(start_idx, end_idx):
        ...
        if dataset_split == 'val':
            re_match = _IMAGE_FILENAME_VALRE.search(image_files[i])
        else:
